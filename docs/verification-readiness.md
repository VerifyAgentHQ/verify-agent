...
| `apps/api`                 | FULLY IMPLEMENTED         | HTTP server (raw Node.js `http`), full wiring, health + verify + protected async result endpoints, plus the Batch 51 single-process GitHub verification service composition.                   |
...
- No durable/background worker daemon, retries, or distributed polling (Batch 51 provides opt-in automatic in-process consumption only)
...
| **Worker**                       | PARTIAL         | `apps/worker/index.ts`                                                                | Validates queue job, delegates to `VerificationApplicationService.verifySource()`                                                                            | Automatic consumption is in-process only (Batch 51 opt-in loop with start/stop); no retry, no durable polling, no distributed workers           |
...
In-memory queue enqueue (IMPLEMENTED)
   ↓
Worker picks up job (PROVEN via Batch 51 automatic in-process consumption
through the existing `VerificationJobProcessor.process()` path; explicit
manual consumption proven in Batches 48–50; no background daemon, no
durable loop)
  ↓
...
| **48**    | GitHub PR-to-Verification composition proof                                    | **DONE.** Focused integration proof wires authenticated webhook → queue → worker → GitHub source resolver → `VerificationApplicationService.verifySource()` → existing pipeline → `VerificationResult` with immutable head-SHA binding. In-memory queue, manual worker consumption; no durable queue, loop, or GitHub feedback.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| **49**    | In-process job runtime and result observation                                  | **DONE.** Application-owned runtime consumes queued jobs through the existing worker (`start`/`processNext`/`drain`/`stop`, explicit, no polling) and stores completed `VerificationResult` values in a bounded in-memory registry with verification-ID, native job-ID, and originating queue-job-ID lookup. Queue and result identities stay distinct; the mapping is process-local, non-durable, and a foundation for future persistent result storage — not persistence itself. No durable queue, background deployment, retries, or GitHub feedback.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **50**    | Asynchronous result observation (explicitly protected)                         | **DONE.** Focused in-process composition exposes retained results via `GET /verification-jobs/:queueJobId/result` through the provider-neutral `VerificationResultReader` port. Batch 50 is asynchronous result observation that is process-local, bounded, non-durable, explicitly protected (`Authorization: Bearer <internal-token>` via `VERIFY_INTERNAL_RESULT_TOKEN`), and not yet a general public production result API. The queue job ID is a lookup handle, never a credential. `200` = retained result currently available (authenticated only); `401` = missing/invalid token (no existence oracle); `404` = no retained result currently available (not completed, evicted, restarted, unknown) and never means verification never happened. Normal configured API-only startup wires no reader/token so the route is unavailable there. Queue-job ID validation reuses the single domain contract (`isValidVerificationQueueJobId`) with no API-only maximum. No durable queue, database, worker daemon, retries, GitHub writeback, AI, frontend, or deployment infrastructure. |
| **51**    | Runnable in-process GitHub verification service                                | **DONE.** Single-process composition (`apps/api/src/github-verification-service.ts`) shares one queue, one application-owned runtime with automatic event-driven consumption (no manual `processNext()`), one worker, one bounded registry, one protected result reader/API, and one GitHub webhook boundary on a single HTTP server with explicit start/stop lifecycle (`tests/batch-51-github-verification-service.test.ts`). Reuses the Batch 50 protected result route without weakening it; configuration fails closed. In-memory, non-durable, single-process, sequential execution only: no retries, no dead-letter queues, no horizontal workers, no persistent results, no GitHub Checks/statuses/comments/reviews/labels, no production-scale worker deployment.                                                                                                                                                                                                                                                                                                                    |
...
## 20. Batch 50: explicitly protected asynchronous result observation

Batch 50 proves the smallest stable in-process composition:

```text
authenticated GitHub PR
        ↓
queueJobId
        ↓
runtime
...
protected result reader
        ↓
GET /verification-jobs/:queueJobId/result
```

Batch 50 is asynchronous result observation that is process-local,
bounded, non-durable, explicitly protected, and not yet a general public
production result API.

- Protection: `Authorization: Bearer <configured-internal-result-token>`
  (`VERIFY_INTERNAL_RESULT_TOKEN`). Exact timing-safe comparison. No
  query-string/path token, no queue ID as credential, no custom header.
  The token is never echoed or logged and never appears in
  `VerificationResult`.
- Fail-closed: the route is enabled only when BOTH a
  `VerificationResultReader` and a non-empty internal token are supplied.
  Reader-without-token (or token-without-reader) yields `404 route not
found`, never a public endpoint. The API-only `startConfiguredApiServer()`
  wires neither, so async result observation is unavailable there; the
  Batch 51 `startConfiguredGitHubVerificationService()` wires the reader
  plus the configured internal token into the same single-process service
  without weakening the protection.
- Responses: missing/invalid auth → generic `401` regardless of whether
  the queue job exists (no existence oracle); retained result →
  `200`; unknown/evicted/not-yet-retained → `404 no retained result`
  (never claims verification never happened); malformed ID → `400`;
  wrong method → `405`; reader failure → safe `500` with no secret/path
  leak. GET never executes verification, dequeues, starts the runtime,
  or touches the sandbox.
- Identity: the webhook `202` returns the actual
  `VerificationQueueJob.jobId`; the result projection keeps
  `queueJobId`, `verificationId`, `jobId`, and `snapshotId` distinct.
- Validation: queue-job IDs reuse the single domain contract
  (`isValidVerificationQueueJobId` in
  `packages/domain/src/verification-queue.ts`); the API accepts every
  domain-valid ID with no API-only maximum length.
- Projection: the existing safe field subset is unchanged (no secrets,
  credentials, webhook secrets, stack traces, request bodies, filesystem
  paths, sandbox commands, environment variables, or raw exceptions).

## 21. Overclaim warning
