...
In-memory queue enqueue (IMPLEMENTED)
   ↓
Worker picks up job (PROVEN via Batch 49 runtime-owned consumption through
`VerificationJobProcessor.process()`; manual consumption proven in Batch 48;
no background loop)
  ↓
...
found`, never a public endpoint. Normal `startConfiguredApiServer()`
  wires neither, so async result observation is unavailable there.
